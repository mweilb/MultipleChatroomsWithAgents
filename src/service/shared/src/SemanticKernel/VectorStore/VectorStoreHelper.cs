    ...
    public partial class VectorStoreHelper<T> where T : TextParagraph, new()
    {
            ...
            string[] questions)
        {
            ...
            foreach (var question in questions)
            {
                ...
                // Optionally post-process the answer (e.g. remove artifacts).
                embeddedText = OllamaHelper.RemoveThinkContent(embeddedText);

                if (!string.IsNullOrWhiteSpace(embeddedText))
                {
                    // Upload an embedding entry for this question.
                    await dataUploader.GenerateEmbeddingsAndUpload<T>(
                        collectionName,
                        documentIdentifier,

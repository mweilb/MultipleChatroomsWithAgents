    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            string configYmlPath = !string.IsNullOrEmpty(configLocation)
                ...
                : "Contexts\\configuration.yml";


            // Use the iterator pattern for the process
            var iterator = new YamlFixProcessIterator(kernel, configYmlPath);

            await foreach (var evt in iterator.IterateAsync(yamlPath))
            {
                ...
                switch (evt.Id)
                {
                    ...
                    case var id when id == ProcessEvents.WaitingOnHumanFinished:
